...
async function main() {
  ...
  for (const filename of files) {
    if (!/^COMP_\d{4,10}\.json$/.test(filename)) throw new Error(`Nombre de archivo no válido: ${filename}`);
...
main().catch((error: unknown) => {
  ...
  if (error instanceof CompanyDataError) error.issues.forEach((issue) => console.error(issue));
  process.exitCode = 1;
});
