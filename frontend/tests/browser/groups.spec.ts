...
test("el acceso aparece automáticamente solo en empresas con grupo", async ({ page }) => {
  ...
  await page.getByRole("link", { name: "Ver inteligencia de grupo GROUP_0042", exact: true }).click();
  ...
  await expect(page.getByRole("link", { name: /Ver inteligencia de grupo/ })).toHaveCount(0);
});
