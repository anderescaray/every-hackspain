...
test("un menú reúne las cuatro secciones de empresa y las tres del grupo", async ({ page }, testInfo) => {
  ...
  await expect(company.getByRole("link")).toHaveCount(4);
  await expect(group.getByRole("link")).toHaveCount(3);
  for (const label of ["Health Score", "Tendencia", "Origen de la caja", "Tiempo financiado"]) await expect(company.getByRole("link", { name: label, exact: true })).toBeVisible();
...
test("los accesos desplazan a la sección elegida sin quitar alertas o escenarios", async ({ page }) => {
  await page.goto("/companies/COMP_0356");
  for (const [label, id] of [["Tendencia", "trajectory"], ["Origen de la caja", "cash-truth"], ["Tiempo financiado", "time-borrowed"], ["Health Score", "health-score"]]) {
    await openMenu(page);
