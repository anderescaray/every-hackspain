...
import type { PulseEnvelope } from "../../types/pulse";
import { fixtureEnvelope } from "./pulseData";

// Publication helper only for authored loader regressions, never used at runtime.
export async function writeFixtureSnapshot(root: string, documents: Record<string, unknown>, envelope: PulseEnvelope = fixtureEnvelope, exporterVersion: "pulse-frontend-v1" | "pulse-frontend-v1.1" = "pulse-frontend-v1") {
  ...
  const manifest = JSON.stringify({ schema_version: "1.0", ...envelope, source_manifest_sha256: "b".repeat(64), exporter_version: exporterVersion, outputs_sha256: hashes, counts: {} });
