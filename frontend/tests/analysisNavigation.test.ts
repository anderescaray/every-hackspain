...
test("empresa tiene acceso a Palancas sin perder Origen de la caja", () => {
  assert.deepEqual(companySections.map((section) => section.id), ["health-score", "trajectory", "actionability", "cash-truth", "time-borrowed"]);
});
