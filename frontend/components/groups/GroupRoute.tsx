...
export async function GroupRoute({ groupId, view, initialRelation, initialCompany, contextCompany }: { groupId: string; view: GroupView; initialRelation?: string; initialCompany?: string; contextCompany?: string }) {
  ...
  return <AnalysisShell companyId={companyId} groupId={groupId} view={view}>
    {!group ? <main className={`${base.page} ${base.routeState}`}><span className={base.eyebrow}>Embat Pulse · Inteligencia de grupo</span><h1>{invalid ? "Los datos del grupo necesitan revisión." : "Datos de análisis del grupo todavía no disponibles."}</h1><p>{groupId} · {invalid ? "El archivo no cumple el contrato esperado. No se muestran conclusiones parciales." : "El análisis solicitado no está disponible. No se inventan sociedades, relaciones ni recomendaciones mientras faltan los datos del grupo."}</p><Link className={base.primaryButton} href={withCompanyContext(`/groups/${groupId}`, companyId)}>Visión general del grupo</Link></main> : <GroupIntelligence key={`${group.group_id}:${view}:${initialRelation ?? ""}:${initialCompany ?? ""}`} group={group} view={view} initialRelation={initialRelation} initialCompany={initialCompany} />}
  </AnalysisShell>;
}
