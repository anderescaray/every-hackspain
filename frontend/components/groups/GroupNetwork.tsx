...
function RelationDetail({ relation, group, onOpen }: { relation: GroupRelation; group: GroupDetail; onOpen: OpenEvidence }) {
  return <>
    <span className={base.eyebrow}>Relación seleccionada</span><h3>{relationKindLabels[relation.kind]}</h3><span className={`${styles.relationBadge} ${styles[relation.status]}`}>{relationStatusLabels[relation.status]}</span>
    <p className={styles.relationEndpoints}>{relationName(relation)}</p><p>{relation.explanation}</p>
    {relation.status !== "identified" && <p className={styles.candidateNotice}>No es una relación confirmada. No permite afirmar apoyo, disponibilidad de recursos ni una dependencia demostrada.</p>}
    <dl className={styles.selectionMetrics}><div><dt>Volumen observado</dt><dd>{groupMoney(relation.volume)}</dd></div><div><dt>Transferencias / pagos</dt><dd>{relation.transfer_count ?? "No disponible"}</dd></div><div><dt>Periodo</dt><dd>{relation.period}</dd></div><div><dt>Recurrencia</dt><dd>{relation.recurrence}</dd></div><div><dt>Cambio</dt><dd>{relationChangeLabels[relation.change]}</dd></div><div><dt>Última observación</dt><dd>{relation.last_seen ? dateLabel(relation.last_seen) : "No disponible"}</dd></div></dl>
    <p className={base.smallText}>Importes y frecuencia suministrados. No sumar relaciones candidatas o desconocidas al apoyo identificado; el volumen no es un saldo disponible.</p><Confidence value={relation.confidence} /><EvidenceButton refs={relation.evidence_refs} title={`Relación ${relationName(relation)}`} onOpen={onOpen} />
    <div className={styles.contextLinks}>{[relation.from_company_id, relation.to_company_id].filter((id): id is string => id !== null).map((id) => <Link key={id} href={`/companies/${id}`}>Ver {id} ↗</Link>)}<Link href={`/groups/${group.group_id}/recommendations?relation=${encodeURIComponent(relation.id)}`}>Revisiones relacionadas →</Link></div>
  </>;
}
...
export function GroupNetwork({ group, onOpen, initialRelation, initialCompany }: { group: GroupDetail; onOpen: OpenEvidence; initialRelation?: string; initialCompany?: string }) {
  ...
  const visible = group.relations.filter((relation) => (status === "all" || relation.status === status) && (kind === "all" || relation.kind === kind) && (company === "all" || relation.from_company_id === company || relation.to_company_id === company));
  const resolved = visible.filter((relation) => relation.from_company_id !== null && relation.to_company_id !== null);
  const unresolved = visible.length - resolved.length;
  ...
  const canvasWidth = Math.max(720, group.members.length * 60);
  const canvasHeight = Math.max(480, canvasWidth * 0.65);
  const positions = new Map(group.members.map((item, index) => {
    ...
    return [item.company_id, { x: canvasWidth / 2 + Math.cos(angle) * (canvasWidth / 2 - 100), y: canvasHeight / 2 + Math.sin(angle) * (canvasHeight / 2 - 80) }];
  }));
  ...
  return <>
    <section className={base.panel} aria-label="Explorar la red del grupo">
      <div className={styles.panelHeading}><div><h2>La red económica observada</h2><p>Las flechas siguen la dirección observada del flujo. Las cuentas de una misma sociedad no se convierten en nodos distintos.</p></div></div>
      <div className={styles.filters}>
        <label>Evidencia de la relación<select aria-label="Evidencia de la relación" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">Todos los estados</option><option value="identified">Identificadas · evidencia alta</option><option value="candidate">Candidatas · por confirmar</option><option value="unknown">Desconocidas</option></select></label>
        <label>Tipo de relación<select aria-label="Tipo de relación" value={kind} onChange={(event) => setKind(event.target.value)}><option value="all">Todos los tipos</option>{Object.entries(relationKindLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
        <label>Sociedad conectada<select aria-label="Sociedad conectada" value={company} onChange={(event) => setCompany(event.target.value)}><option value="all">Todas las sociedades</option>{group.members.map((item) => <option key={item.company_id}>{item.company_id}</option>)}</select></label>
        <button className={base.secondaryButton} onClick={() => { setStatus("all"); setKind("all"); setCompany("all"); setSelection(null); setZoom(1); }}>Restablecer vista</button>
      </div>
      <div className={styles.networkLegend}>{Object.entries(relationStatusLabels).map(([key, label]) => <span key={key}><i className={styles[key]} />{label}</span>)}</div>
      <p className={styles.guardrail}>La ausencia de una conexión no significa ausencia de relación. Los extremos no identificados se mantienen en la lista, sin inventar una sociedad ni dibujar un enlace.</p>
      <div className={styles.networkLayout}>
        <div className={styles.graphPanel}>
          <div className={styles.graphToolbar}><span role="status">{visible.length} relaciones visibles · {unresolved} sin extremos completos</span><div><button aria-label="Reducir zoom de la red" disabled={zoom <= 0.6} onClick={() => setZoom(Math.max(0.6, zoom - 0.2))}>−</button><output aria-label="Zoom de la red">{numberLabel(zoom * 100, 0)} %</output><button aria-label="Ampliar zoom de la red" disabled={zoom >= 1.6} onClick={() => setZoom(Math.min(1.6, zoom + 0.2))}>+</button></div></div>
          {group.members.length ? <div className={styles.graphViewport} tabIndex={0} role="region" aria-label="Lienzo desplazable de la red">
            <svg width={canvasWidth * zoom} height={canvasHeight * zoom} viewBox={`0 0 ${canvasWidth} ${canvasHeight}`} role="group" aria-label="Grafo interactivo de sociedades y relaciones">
              ...
              {resolved.map((edge, index) => {
                ...
                return <g key={edge.id} role="button" tabIndex={0} aria-label={`Seleccionar relación ${relationName(edge)}: ${relationStatusLabels[edge.status]}`} aria-pressed={relation?.id === edge.id} className={styles.graphEdge} onClick={() => selectRelation(edge.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); selectRelation(edge.id); } }}>
                  <title>{`${relationName(edge)} · ${relationKindLabels[edge.kind]} · ${relationStatusLabels[edge.status]}`}</title>
                  <path d={path} fill="none" stroke="transparent" strokeWidth="20" />
                  <path d={path} fill="none" stroke={edge.status === "identified" ? "#3368bd" : edge.status === "candidate" ? "#92651d" : "#677488"} strokeWidth={relation?.id === edge.id ? 4 : 2} strokeDasharray={edge.status === "candidate" ? "8 5" : edge.status === "unknown" ? "2 6" : undefined} markerEnd={`url(#${graphId}-${edge.status})`} />
                </g>;
              })}
          ...
          </div> : <p className={base.emptyState}>No hay sociedades observadas para dibujar una red.</p>}
          <p className={styles.graphHint}>Selecciona una sociedad o una flecha; también puedes usar Tab y Enter o la lista de relaciones. Desplaza el lienzo o ajusta el zoom si lo necesitas.</p>
          {!resolved.length && group.members.length > 0 && <p className={base.emptyState}>No hay conexiones dibujables con estos filtros. Esto no demuestra ausencia de relación.</p>}
        </div>
        <aside ref={detailPanel} id="group-network-selection" className={styles.selectionPanel} aria-label="Detalle de la selección" aria-live="polite">
          {member ? <MemberDetail member={member} group={group} onOpen={onOpen} /> : relation ? <RelationDetail relation={relation} group={group} onOpen={onOpen} /> : <><span className={base.eyebrow}>Explora la red</span><h3>{selection ? "La selección no está disponible en esta vista" : "Cada conexión tiene una explicación"}</h3><p>{selection ? "Comprueba los filtros o selecciona otro elemento. No se sustituye por una relación diferente." : "Selecciona un nodo para revisar la sociedad, o una relación para ver su dirección, volumen, recurrencia y evidencia."}</p><p className={base.smallText}>No todas las transferencias internas son apoyo. Una relación comercial y una candidata a centralización permanecen diferenciadas.</p></>}
        </aside>
      </div>
      <section className={styles.relationshipList} aria-label="Lista de relaciones">
        <h3>Relaciones observadas y pendientes</h3><p className={base.smallText}>Vista alternativa al grafo. Incluye contrapartes no identificadas, sin atribuirlas a sociedades ficticias.</p>
        {visible.map((edge) => <button key={edge.id} className={styles.relationshipRow} aria-pressed={relation?.id === edge.id} onClick={() => selectRelation(edge.id)}><span><strong>{relationName(edge)}</strong><small>{relationKindLabels[edge.kind]} · {edge.recurrence}</small></span><span className={`${styles.relationBadge} ${styles[edge.status]}`}>{relationStatusLabels[edge.status]}</span><span>{groupMoney(edge.volume)}<small>{relationChangeLabels[edge.change]}</small></span><span aria-hidden="true">→</span></button>)}
        {!visible.length && <p className={base.emptyState}>No hay relaciones suministradas que coincidan con estos filtros.</p>}
      </section>
    </section>
    <Link className={styles.nextStep} href={`/groups/${group.group_id}/recommendations`}><span><small>Siguiente paso</small><strong>Qué debería revisar tesorería</strong></span><span aria-hidden="true">→</span></Link>
  </>;
}
