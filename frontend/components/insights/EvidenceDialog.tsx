...
import type { EvidenceGroup, ObservationEvidence } from "@/types/companyDetail";
        ...
        {groups.map((group) => {
          ...
          return (
            <section className={styles.evidenceGroup} key={group.id}>
              ...
              {transactions.length > 0 && <div className={styles.tableScroll} tabIndex={0} role="region" aria-label={`${group.title}: movimientos`}><table><caption>Movimientos representativos · EUR</caption><thead><tr><th scope="col">Movimiento / fecha</th><th scope="col">Categoría</th><th scope="col">Descripción</th><th scope="col" className={styles.numeric}>Importe</th></tr></thead><tbody>{transactions.map((row) => <tr key={row.id}><td><span className={styles.mono}>{row.id}</span><small>{dateLabel(row.transaction_date)}</small></td><td><span className={`${styles.category} ${styles[row.category]}`}>{cashCategoryLabels[row.category]}</span></td><td>{row.description}</td><td className={styles.numeric}>{exactMoney(row.amount)}</td></tr>)}</tbody></table></div>}
