    ...
    <section className={styles.panel} aria-label="Tendencia">
      ...
      {!current || !first || !last ? <p className={styles.emptyState}>{observations.length ? "No hay Health Score identificado en el histórico disponible." : "Todavía no hay histórico de Health Score publicado."}</p> : <>
        ...
        <svg viewBox="0 0 760 266" className={styles.chart} role="img" aria-labelledby={`${chartId}-title ${chartId}-description`} onPointerMove={selectFromPointer}>
          ...
          <defs><linearGradient id={`${chartId}-fill`} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#0c7a6c" stopOpacity="0.18" /><stop offset="100%" stopColor="#0c7a6c" stopOpacity="0" /></linearGradient></defs>
          {[0, 25, 50, 75, 100].map((value) => <g key={value}><line x1="44" x2="724" y1={y(value)} y2={y(value)} stroke="#cfd9d2" strokeDasharray={value === 0 ? undefined : "3 5"} /><text x="30" y={y(value) + 4} textAnchor="end" fill="#5a6b64" fontSize="12">{value}</text></g>)}
          <polygon points={`${x(0)},224 ${points} ${x(history.length - 1)},224`} fill={`url(#${chartId}-fill)`} />
          <polyline points={points} fill="none" stroke="#0c7a6c" strokeWidth="3" strokeLinejoin="round" strokeLinecap="round" />
          <line x1={x(activeIndex)} x2={x(activeIndex)} y1="24" y2="224" stroke="#8fb5ab" strokeDasharray="4 4" />
          {history.map((point, index) => <circle key={point.month} cx={x(index)} cy={y(point.health_score)} r={index === activeIndex ? 6 : 3} fill={index === activeIndex ? "#0c7a6c" : "white"} stroke="#0c7a6c" strokeWidth="2"><title>{`${dateLabel(point.month, true)}: Health Score ${point.health_score}`}</title></circle>)}
          <g aria-hidden="true" className={styles.chartTooltip}>
            <rect x={labelX - 22} y={y(current.health_score) - 42} width="44" height="28" rx="7" fill="#0b1f1c" />
          ...
          {history.filter((_, index) => index === 0 || index === history.length - 1 || index % 6 === 0).map((point) => <text key={point.month} x={x(history.indexOf(point))} y="253" textAnchor="middle" fill="#5a6b64" fontSize="12">{dateLabel(point.month, true)}</text>)}
        </svg>
