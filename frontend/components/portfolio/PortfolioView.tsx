...
export function PortfolioView({ portfolio, query, page }: { portfolio: Portfolio; query: PortfolioQuery; page: number }) {
  ...
  return <main className={`${base.page} ${styles.portfolio}`}>
    ...
    <section className={styles.kpis} aria-label="Resumen de la cartera">
      ...
      <Kpi label="Atención alta" value={totals.highAttention} hint="Deterioro confirmado o dependencia de apoyo ≥ 50 %" />
      <Kpi label="Deteriorándose" value={totals.deteriorating} hint="Dirección confirmada o emergente" />
      <Kpi label="Mejorando" value={totals.improving} hint="Dirección confirmada o emergente" />
      <Kpi label="Dependen de apoyo" value={totals.dependent} hint="Apoyo intragrupo ≥ 30 % de las entradas (6 m)" />
    </section>
    ...
    <form className={styles.filters} method="get" action="/" aria-label="Filtros de la cartera">
      ...
      <label>Atención<select name="attention" defaultValue={query.attention}><option value="all">Todas</option>{(["high", "medium", "low"] as const).map((v) => <option key={v} value={v}>{attentionLabels[v]}</option>)}</select></label>
      <label>Estado<select name="status" defaultValue={query.status}><option value="all">Todos</option>{(["scored", "provisional", "not_scored"] as const).map((v) => <option key={v} value={v}>{statusLabels[v]}</option>)}</select></label>
        ...
        <table className={styles.table}>
          ...
          <tbody>
            {rows.map((item) => <tr key={item.company_id} data-attention={item.attention}>
              ...
              <td className={styles.score}>{item.health_score === null ? <span className={styles.muted}>—</span> : <strong>{item.health_score}</strong>}</td>
              ...
              <td><span className={styles.status}>{statusLabels[item.score_status]}</span>{item.status_reason && item.score_status !== "scored" ? <small className={styles.reason}>{item.status_reason}</small> : null}</td>
            </tr>)}
          </tbody>
        </table>
      </div>
