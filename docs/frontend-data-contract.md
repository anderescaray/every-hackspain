...
| `actionability` | Extensión opcional de `company_sensitivity_v1`; solo escenarios ya calculados por el advisor, con `status`, `primary`, `alternatives`, `next_band` y procedencia |

### Actionability / «Qué puede mover tu Health»

`09_export_frontend.py` exige un run contemporáneo de `08_treasury_advisor.py` (`data/processed/advisor/_advisor_manifest.json` y `company_sensitivity/*.json`). Si falta o el mes no coincide con el producto, falla; nunca rellena sensibilidades con V2, What-if ni fixtures. El JSON de empresa incorpora `actionability` sin cambiar el contrato raíz 2.0. Para un score histórico cuya fecha no coincide con el mes del advisor, publica `status: "unavailable"`, `reason: "score_month_not_current"` y `primary: null`.

`actionability.status`: `actionable_treasury` (acción potencial de tesorería), `business_sensitivity_only` (solo sensibilidad de negocio), `structural_issue` (el advisor detecta arrastre operativo y ninguna tesorería alcanza el siguiente tramo), `top_band_no_action` (nivel V2 ya en tramo superior), `no_actionable_lever` (empresa puntuada sin mejora positiva en la rejilla) o `insufficient_data` (score/fecha no comparable). El estado `no_actionable_lever` **no** se presenta falsamente como falta de datos ni como problema estructural. `treasury_actions[]` y `business_sensitivities[]` separan los dos tipos; `primary` y `alternatives` preservan el orden de presentación, pero si `primary.type=business` se titula «Principal sensibilidad», nunca «mejor acción». En tramo superior `primary` es `null`: no se promueve un 98→100 como acción prioritaria.

Cada palanca incluye `lever`, `type`, `label`, `level_before/after`, `health_before/after`, `delta_points`, `quantity`, `change_description`, `required_change` (`absolute`, `relative_pct`, `unit`), `resources`, `cash_equivalent`, `efficiency`, `horizon`, `next_breakpoint`, `source_grid_rel_change` y `scenario_id`. Los cambios requeridos son la diferencia entre magnitudes **ya calculadas por el advisor** y la proporción de su fila de rejilla, expresadas por el adaptador Python. La UI no los calcula. `null` permanece `null`; `scenario_id` es `null` porque las primitivas del advisor no equivalen exactamente a los escenarios existentes de What-if. La UI puede desplazarse al simulador, pero no seleccionar una aproximación arbitraria.

Se presenta la fila de rejilla con mayor aumento **positivo de Health a k=6** por palanca (empates: menor cambio relativo). Para la principal se prefieren palancas de tesorería con `level_per_10k` positivo según el ranking del advisor; de no haberlas, tesorería por impacto modelado; de no haber tesorería útil, la mejor sensibilidad de negocio, marcada explícitamente como tal. Si el advisor confirma arrastre operativo y ninguna palanca de tesorería alcanza el siguiente tramo, se publica `structural_issue` y la sensibilidad de negocio puede ser principal. No se extrapolan niveles, scores ni pendientes. `efficiency` conserva la pendiente marginal del advisor en **puntos de nivel por €10k**, no equivale al delta del escenario ni a puntos de Health. Solo se muestra para AP («Eficiencia de liquidez») o reducción del servicio de deuda («Eficiencia de caja») con equivalente de caja válido; no se promociona una eficiencia monetaria de una hipótesis de negocio como si fuese acción ejecutable.

`resources` solo se informa para AP y describe **la fila de rejilla seleccionada**: `required` = `grid.cash_equivalent` si es positivo y está identificado, `own_available` = caja excedente del advisor, `gap` y `feasibility` derivados de esos dos importes. Sin caja fiable, `own_available/gap` son `null` y la factibilidad es `unknown`. Un equivalente AP igual a cero por proxy de stock vencido/no observado **no prueba coste cero** de acelerar pagos: en ese caso `required/gap=null` y factibilidad `unknown`, aunque el valor crudo cero permanezca en `cash_equivalent` para auditoría. No se muestra el `cash_needed` de liquidar todo el AP como si fuese el coste de la fila parcial. `band_basis: "level_v2"` deja explícita la base del tramo. `next_band` conserva `current_level`, `target_level` y `projected_level` para trazabilidad, y exporta además `current_health`, `projected_health` y `target_health` para el camino visible. El objetivo Health se obtiene **en Python** aplicando la misma función existente del advisor `score_with_baseline_momentum(target_level, baseline.momentum_adjustment)`; el momentum permanece fijo, como en los escenarios. `reachable_with_primary` sigue describiendo el cruce del tramo **de nivel**, no un nuevo tramo de Health. Si la saturación deja el objetivo Health sin distancia visible, `next_band=null` en vez de dibujar una falsa trayectoria. El tramo superior proviene de `next_tramo_target=null` del advisor (configuración 40/70 sobre nivel), no de un umbral Health inventado; por eso puede haber empresas con Health<70 y nivel ya en el tramo superior. `horizon.k1_level_delta` y `k6_level_delta` son del nivel; el advisor no calcula Health k1. Se trata de sensibilidad mecánica, no de recomendación ejecutable ni forecast; las palancas de negocio no se etiquetan como acciones de tesorería.
