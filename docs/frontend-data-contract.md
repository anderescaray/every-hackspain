...
`actionability.status`: `available` (palanca de tesorería), `business_sensitivity` (solo sensibilidad de negocio), `no_actionable_lever` (ninguna rejilla con impacto positivo) o `unavailable` (sin score/fecha comparable). `primary` y cada elemento de `alternatives` comparten contrato: `lever`, `type`, `label`, `level_before/after`, `health_before/after`, `delta_points`, `quantity`, `resources`, `cash_equivalent`, `efficiency`, `horizon`, `next_breakpoint`, `source_grid_rel_change` y `scenario_id`. `null` permanece `null`; `scenario_id` es `null` porque las primitivas del advisor no equivalen exactamente a los escenarios existentes de What-if. La UI puede desplazarse al simulador, pero no seleccionar una aproximación arbitraria.

Se presenta la fila de rejilla con mayor aumento **positivo de Health a k=6** por palanca (empates: menor cambio relativo). Para la principal se prefieren palancas de tesorería con `level_per_10k` positivo según el ranking del advisor; de no haberlas, tesorería por impacto modelado; de no haber tesorería útil, la mejor sensibilidad de negocio, marcada explícitamente como tal. No se extrapolan niveles, scores ni pendientes. `efficiency` conserva la pendiente marginal del advisor en **puntos de nivel por €10k**, no equivale al delta del escenario ni a puntos de Health. Solo aparece cuando existe equivalente de caja válido: AP como «Eficiencia de liquidez», deuda/salidas como «Eficiencia de caja»; AR y entradas no reciben eficiencia inventada.

`resources` solo se informa para AP y describe **la fila de rejilla seleccionada**: `required` = `grid.cash_equivalent`, `own_available` = caja excedente del advisor, `gap` y `feasibility` derivados de esos dos importes. Sin caja fiable, `own_available/gap` son `null` y la factibilidad es `unknown`. No se muestra el `cash_needed` de liquidar todo el AP como si fuese el coste de la fila parcial. `next_band` opera sobre el **nivel V2** (`current_level`, `target_level`, `projected_level`, `reachable_with_primary`), no sobre el Health ajustado por momentum. `horizon.k1_level_delta` y `k6_level_delta` son del nivel; el advisor mantiene momentum en baseline, y no calcula Health k1. Se trata de sensibilidad mecánica, no de recomendación ejecutable ni forecast; las palancas de negocio no se etiquetan como acciones de tesorería.
