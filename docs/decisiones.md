...
## 22. Rama `score_v2`: Pulse v1.2 sobre main D32, contrato web único — 19-09-2026

**Supersede la elección de presentación de §21 para esta rama, sin reescribir su contexto histórico.** El `main` fusionado aporta conversión D32 a EUR, limpieza revisada, advisor y simulador V2. Sobre esa base se selecciona PulseFourPillars como fuente **exclusiva** del frontend: `scripts/09_export_frontend.py` verifica un run Pulse y publica `current.json` + snapshot inmutable; el loader Next fija y verifica los mismos `run_id`, versiones y SHA-256 en empresa, grupo y cartera. `financial_smoothed_v2` sigue disponible en `score_v2/` y `src/xray/product/legacy_frontend_export.py` para comparación y el advisor; no es fallback de la web. `scripts/09b_export_frontend_pulse.py` comparte el exportador canónico, no otra interpretación.

D32 altera el universo económico pese a conservar las fórmulas de Pulse v1.1. Se registra `PulseFourPillars-v1.2` / `pulse-config-v1.2` con `cleaning-v2`, `cash-truth-v2`, `monthly-facts-v2` y la misma composición `operating-extended-health-v1`. Operating Health (G/M/R) es la nota principal de Portfolio; Extended Health sólo existe con Debt identificado `verified` o `bounded`. El frontend no recalcula, imputa ni usa V2 si falta Debt. El catálogo de FX está fechado/hashado en clasificación, declara fecha de conocimiento 2026-09-19 y el archivo `xray/fx.py` se incluye en el código del run. Un `as_of=2026-08-31` v1.2 requiere `data_vintage>=2026-09-19` y es retrospectivo, no conocido al cierre ni comparable con v1.1 como puro cambio económico. Los artefactos v1.1 pre-D32 permanecen históricos; no se publican en una web basada en D32. Ejecutar un nuevo run y exportar su snapshot antes de validar la UI.
