...
| D41 · estrés propio y anticipación medida | **Decidida por el usuario el 2026-09-19. No cambia puntuaciones** (0 diferencias en V2). (A) `event_type` (D30) gana el tipo `embargo_tercero`, con prioridad sobre `embargo`: ingresos en Hacienda de lo retenido a un empleado o proveedor ("DOC. DE INGRESOS ASOCIADOS EMBARGOS"), retenciones de nómina y transferencias al juzgado. Son 331 movimientos en 47 empresas; quedan 315 embargos propios en 129 ("EMBARGO COMUNICADO", "EJECUCION EMBARGO", "EMBARGOS AEAT"…). (B) `src/xray/evaluation/lead_time.py` + `scripts/11_lead_time.py` → `data/processed/evaluation/lead_time.{json,md}` y `lead_time_events.parquet`. Reglas fijadas antes de ver resultados: evento = primer estrés propio (cuota impagada, embargo propio, aplazamiento, descubierto, recargo de apremio, demora) tras 6 meses observados sin estrés; señal = primera etiqueta V2 `emerging_deterioration`/`deteriorating` en los 6 meses previos (umbral fijo de V2); evaluable con ≥3 meses de score previos; falsa alarma = inicio de racha de alarma sin estrés propio en los 6 meses siguientes (solo alarmas con 6 meses de futuro observado). **Resultado:** 153 inicios (142 evaluables, 11 censurados); detectados 45 (32%); antelación mediana 3 meses (p25–p75: 2–5); 78% de falsas alarmas; evento tras alarma 22% frente al 18% de base (×1,2): **señal débil, no se maquilla**. Las empresas con estrés propio ya puntúan 3–5 puntos menos que el resto seis meses antes (estudio de eventos): el score las ve más débiles como nivel, no como caída que se acelera. Las 123 empresas que dejan de tener datos casi nunca tienen estrés propio antes de irse (2–4% frente al 16% de las activas): 53 cortan de golpe (baja en Embat), 52 se apagan (cobros al 5%; cese o cambio de banco), 15 intermedias, 3 sin historia; no se usan como evento de validación (circular: se definen con los mismos flujos que usa el score)  **Ampliado (20-09): las dos direcciones y el cuadro por empresa.** Mejora: recuperación = primer mes de 6 observados sin estrés tras haberlo tenido; 143 recuperaciones (94 evaluables, 49 censuradas), 24 avisadas con `improving`/`emerging_improvement` (26%), antelación mediana 3 meses (2–4), y el 20% de las señales de mejora van seguidas de estrés igualmente. Por empresa: de 1.286, 267 tienen estrés propio y 132 lo tienen con score previo; **avisamos antes del evento a 44 (33%)**; 493 reciben alguna alarma de deterioro alguna vez, de las que 343 nunca llegan a tener estrés; 401 reciben alguna señal de mejora. |
| D42 · alertas tempranas comparadas | **Diagnóstico, no cambia nada** (`src/xray/evaluation/early_warning.py`, `scripts/12_early_warning.py` → `evaluation/early_warning.{json,md}`). Mismo evento, ventana y censura que D41; solo cambia el disparador: `momentum_z ≤ −X` (X = 0,5/1/1,5/2), `cash_runway < Y` meses (Y = 0,5/1/2), `caja < 0` y la unión e intersección de score y caja. Cada alerta se evalúa donde existe su señal, y además todo se repite sobre el **subconjunto común** de 43 eventos: es la única comparación limpia. Se publica `alarm_rate` (proporción de meses en alarma) para desenmascarar alarmas triviales. **Resultados (43 eventos comunes):** `runway<1m` detecta el 93% pero está encendida el 53% de los meses y su lift es ×1,1 (trivial: la mediana de caja de la cartera es 0,69 meses); `score_z ≤ −1` detecta 67% con 20% de meses en alarma (×1,2); `score_z ≤ −1,5` (umbral vigente) 47% con 12%; a igual coste (~7% de meses en alarma) la **intersección score+caja** detecta 35% frente al 28% del `score_z ≤ −2`, con mejor lift (×1,5 frente a ×1,3) pero menos antelación (2 frente a 3,5 meses); `caja < 0` es la de mejor lift (×1,7) y solo 14% de detección. **Conclusión: la caja sola no mejora al score; la combinación aporta algo de precisión a cambio de avisar más tarde. Ninguna alerta pasa de ×1,7 sobre la tasa base.** Elegir punto de trabajo es decisión de producto, no se calibra aquí |
| D43 · alerta de presión de deuda | **Mejor alerta encontrada; es diagnóstico y producto, no cambia el score.** Regla: el servicio de deuda (principal + intereses) pasa a comerse **más de 5 puntos porcentuales** de las entradas operativas frente a la **mediana propia de los 6 meses anteriores** (mínimo 3 meses observados; solo pasado, `shift(1)`). `debt_pressure_alarms` en `evaluation/early_warning.py`. **Procedimiento anti-autoengaño:** los 250 grupos se partieron 70/30 con semilla fija; se iteró (12 señales candidatas, umbrales, votaciones 2-de-3, uniones e intersecciones) **solo en los 175 grupos de desarrollo** y la regla elegida se midió **una vez** en los 75 de reserva. **Resultado:** desarrollo 44% de detección, 9% de meses en alarma, ×2,2, 5 meses de antelación; **reserva 44%, 9%, ×1,8, 3,5 meses** (encogimiento normal del lift). Sobre toda la cartera: 128 eventos evaluables, 44%, 9%, ×2,1, 65% de falsas alarmas (frente al 78% de la alarma vigente del score) y 1.132 empresas cubiertas. Mejora la alerta vigente `score_z ≤ −1,5` (43–46%, 12–14% de meses, ×1,1–1,3). **Qué se descartó y por qué:** ni la cuota sola (×0,73) ni la caída de ingresos sola (×0,92) sirven, es el cociente; dejar de pagar impuestos o Seguridad Social no anticipa (×0,74–0,88); el retraso AP (×0,98) y el AR (×0,65) tampoco; ninguna votación ni combinación superó a la regla simple; `runway<1m` detecta el 93% pero está encendida el 53% de los meses (×1,1, alarma trivial). Por tipo, detecta mejor cuota impagada (80%) y recargo de apremio (80%) que embargo (29%). **Sigue siendo señal modesta:** 2 de cada 3 alarmas no acaban en evento |
| D44 · monitor de alertas | **Nuevo, no cambia el score** (`src/xray/product/monitor.py`, `scripts/13_monitor.py` → `data/processed/monitor/alerts.{parquet,json}`). Responde al bonus "monitor que avisa": lista del último cierre ordenada por severidad, con motivo, evidencia numérica, racha de meses encendida y el rendimiento **medido** de cada tipo de alerta. Dos tipos: `presion_deuda` (D43) y `deterioro_confirmado` (etiqueta `deteriorating` de V2). Agosto 2026: **87 alertas en 85 empresas** (70 presión de deuda, 17 deterioro confirmado, 2 empresas con las dos). La severidad de la presión de deuda se acota al 500% de carga solo para mostrarla (una empresa con entradas casi nulas daría cifras absurdas); la alarma usa el valor sin acotar. Candidata directa para `alerts[]` de la ficha y una columna en Portfolio |
| D45 · lo que se probó y no mejoró | **Registro de resultados negativos (búsqueda de literatura, 19-09).** Se implementaron y midieron con el protocolo de D41, iterando solo en los grupos de desarrollo: (a) **modelo de riesgo en tiempo discreto** (regresión logística mensual con ridge, IRLS propio en numpy; el estándar del *behavioural scoring*, Bellotti & Crook): con 10 variables solo cubre 729 de 12.048 meses en riesgo; compacto a 3–5 variables de alta cobertura llega a 43% de detección con 11% de meses en alarma (×1,82), **por debajo** de la regla D43 (44% con 9%, ×2,20); suavizarlo a 3 meses empeora. (b) **CUSUM** (k = 0,25–0,75; h = 1,5–3) y **EWMA** (λ = 0,2–0,3; L = 1,5–2,5) sobre la carga de deuda estandarizada contra la propia historia: detectan 41–55% pero quedan encendidos el 24–39% de los meses (×1,37–1,53). (c) Variantes de la regla: base de 12 meses (×2,26 con 10%), confirmación 2 de 3 meses (39% con 8%, ×2,46: **punto de trabajo más preciso, disponible si se prefiere menos ruido**), solo intereses (15%, ×1,78) y persistencia de 3 meses (46% pero 17% de alarma). **Conclusión: la regla simple y explicable sigue siendo la mejor relación detección/coste; se documenta para no repetir el intento** |
| D46 · cuentas dormidas con saldo cero | **Decidida por el usuario el 2026-09-20; auditada desde raw el mismo día.** Una cuenta corriente sin ningún movimiento en toda la ventana y con saldo 0 se da por **fiable en 0** al reconstruir la caja: no puede esconder liquidez. Con saldo distinto de 0 sigue siendo no fiable (no sabemos su historia). Antes bastaba una de estas cuentas para dejar a la empresa entera sin caja calculada. **Reproducción exacta:** caja fiable en agosto 928 → **1.066 empresas** (+138); meses-empresa con caja 14.928 → 16.431; los artefactos V1 y V2 son idénticos byte a byte porque consumen `company_monthly_features`, no el contexto de liquidez; en el advisor `donor_cash_unreliable` cae de 3.600 a 1.874 pares. Siguen sin caja 220 empresas en agosto; causa primaria exclusiva: 105 con cuenta dormida de saldo distinto de cero, 88 con snapshot/saldo ausente, 17 con movimiento futuro inseguro, 3 con saldo centinela, 2 con snapshot antiguo y 5 sin cuenta corriente observada |
| D47 · oportunidades de tesorería del grupo | **Hechos medidos, no escenarios; auditada y corregida el 2026-09-20** (`src/xray/product/group_opportunities.py`, `scripts/14_group_opportunities.py` → `data/processed/group_opportunities/`). Las cifras publicadas inicialmente **no se reproducían** y dos métricas estaban mal definidas: el bruto de netting sumaba las dos patas espejo y el bloque de descubiertos llamaba «caja de hermana» a la caja de la propia filial. Cifras corregidas: (1) **cash pooling:** 1.256 meses-empresa negativos (236 empresas, 112 grupos); 1.160 son evaluables porque hay ≥1 hermana con caja fiable observada, y 991/1.160 = **85,4%** son cubribles (déficit mediano 9.112 €); 96 quedan como no evaluables, no como cero. De 219 meses con descubierto (95 empresas), 179 tienen cobertura de hermanas y 175 tienen una hermana con caja: 79,9% del total / 97,8% de los evaluables; 40 no son evaluables y 26 falsos positivos de la versión inicial eran caja propia. Coste observado: 17.823 €. (2) **netting:** 43.819 transferencias (87.638 apuntes) y 21.471 M€ brutos → 1.602 M€ netos, **92,5% menos**, en 153 grupos; quedan fuera 97/250 grupos (71 unipersonales y 26 multigrupo sin flujo detectado). (3) **crédito rotativo sin usar:** 54 filiales negativas al cierre, 25 cubribles con crédito de hermanas (mediana 1.323.958 €). No toca el score. Límites: caja reconstruida y retrospectiva, crédito disponible a extracción y fiscalidad/legal fuera de los datos |
| D48 · palanca O del advisor (pagos operativos) | **Nueva, opcional y auditada el 2026-09-20** (`--levers D1,P,O`; por defecto sigue `D1,P`). La donante asume la fracción ω de los **pagos operativos** de la receptora: receptora `outflow_scale(−ω)`, donante `outflow_add(+ω·o_b)`, con κ=2 meses, δ=5 puntos y rejilla 25/50/75/100%. Se reproducen los bloqueos base (`recipient_no_debt_service` 3.640; `recipient_ap_component_unavailable` 3.472) y los resultados: planes **19 → 48** (29 nuevos, 10 con una filial roja); 106 pasos O, importe mediano 40.694 €; cambio mediano de nivel por paso O +9,73 receptora / −0,21 donante. ΔG k=6 por plan: min 0,25 · p25 0,87 · mediana 2,49 · p75 5,06 · máx 22,06; k=1 mediana 0,20, aunque 3 planes empeoran marginalmente a k=1. La peor filial mejora en 23/48 planes: +11,83 de media entre esos 23, o +5,67 promediando los 48 (la cifra inicial «media +5,7» usaba este segundo denominador), máximo +30,71. Replay de los 124 pasos: 0 violaciones de colchón R2 (holgura mínima 883,75 €) y 0 de suelo R5 (holgura mínima 0,013 puntos); aun así, δ permite que una donante (`COMP_0640`, `GROUP_0022`) cruce de ámbar a rojo. **Límite financiero cuantificado:** 6/48 planes son mecánicamente válidos pero económicamente absurdos bajo el criterio conservador «una receptora gana nivel con ≤1.000 € de entradas operativas en seis meses» (`GROUP_0016`, `GROUP_0070`, `GROUP_0102`, `GROUP_0116`, `GROUP_0142`, `GROUP_0179`); son 11/48 con umbral 10.000 €. En 1 (`GROUP_0070`) desaparece estrictamente un componente al retirar deuda sin entradas. No se introduce un umbral nuevo sin decisión de producto. El gasto externo del grupo no cambia: cambia quién lo soporta |
...
### GA-08 · Perfil seguro para la UI (`production_safe`, 20-09-2026)

**Hecho:** D48 demostró que el motor mecánico es correcto (R2/R5 0 violaciones) y que **no es un recomendador de producción**: 6/48 planes D1+P+O tenían receptoras con ≤1.000 EUR de entradas en 6 meses; 3 planes empeoraban G a k=1; al menos un donante bajaba de tramo dentro de δ=5; 25/48 no mejoraban a la peor filial. El perfil `AdvisorConfig.production_safe=True` recorta el mismo motor sin cambiar V1/V2 ni el artefacto experimental `data/processed/advisor/` (sigue D1,P por defecto).

| Parámetro | Valor | Motivo |
|---|---|---|
| `production_safe` | `False` por defecto | El laboratorio conserva los 19/48 planes; la UI no los consume |
| Palancas en `--production-safe` | `D1,P,O` | O ataca el bloqueo de deuda/facturas; sigue siendo escenario, no ejecución |
| `min_recipient_inflow_6m` | 10.000 EUR | Suelo no calibrado contra el artefacto GA-06 (358 EUR de entradas / cuota residual). No es un umbral de «empresa real» |
| `min_recipient_inflow_outflow_ratio` | 1 % | Evita receptoras cuyo outflow de ventana hace insignificante cualquier entrada |
| Actividad (D1/O) | `recipient_activity_below_production_floor` / `_ratio_…` | P ya exige retraso AP y restricción de liquidez |
| `protect_donor_tramo` | True | Prohíbe ámbar→rojo o verde→ámbar del donante aunque δ=5 lo admita |
| `require_nonnegative_k1` | True | Exige ΔG ≥ 0 también en el próximo cierre |
| `require_worst_level_improvement` | True | Cada paso tiene que subir el mínimo de las filiales puntuadas |
| Componente que desaparece | `recipient_component_would_disappear` | Evita el salto 30→40 al anular deuda sin entradas |
| Salida | `data/processed/advisor_production/` | `08 --production-safe`; `09` solo mapea planes con `config.production_safe` a `recommendations[]` (`id` `advisor-NN-d1\|p\|o`) |

Vocabulario de producto: «escenario para revisión», nunca «ejecutar» ni «transferir». X Ray no mueve fondos. Los umbrales no están calibrados con impagos reales; si no hay plan, la UI dice que ninguna acción supera las guardas, no que el grupo esté sano.

El contrato de grupo 1.0 no cambia: `confidence` del escenario es `null`, `evidence_refs` vacío (el anclaje vive en el JSON del plan), `relation_refs` vacío. La ficha de ejemplo GROUP_0042 añade un `advisor-01-d1` para la demo; los JSON reales salen de `09` contra `advisor_production/`.

Cifras de agosto 2026 del perfil seguro (`08 --production-safe` → `advisor_production/`, 148 s): **19 planes / 39 pasos** (32 O, 6 D1, 1 P) frente a 48/106 del D48 experimental con las mismas palancas. 160 grupos sin palancas factibles y 71 unipersonales. **0** ΔG k=1 negativos, **0** donantes que bajan de tramo, **0** pasos D1/O con entradas <10.000 EUR, **39/39** pasos mejoran el mínimo de las filiales puntuadas, **0** fallos de anclaje. Casos útiles para la UI: `GROUP_0067` (COMP_1275 31,3 rojo → 45,2 ámbar con O de COMP_1048); `GROUP_0016` (COMP_1029 41,9 → 78,9). El recorte 48→19 es el filtro de producto, no un fallo del motor. El artefacto experimental `advisor/` no se recalibra.
