...
| D46 · cuentas dormidas con saldo cero | **Decidida por el usuario el 2026-09-20.** Una cuenta corriente sin ningún movimiento en toda la ventana y con saldo 0 se da por **fiable en 0** al reconstruir la caja: no puede esconder liquidez. Con saldo distinto de 0 sigue siendo no fiable (no sabemos su historia). Antes bastaba una de estas cuentas para dejar a la empresa entera sin caja calculada. **Resultado:** caja fiable en agosto 928 → **1.066 empresas**; meses-empresa con caja 14.928 → 16.431; **0 diferencias en V1 y V2**; en el advisor los bloqueos por `donor_cash_unreliable` caen de 3.600 a 1.874 pares |
| D47 · oportunidades de tesorería del grupo | **Nuevo, hechos medidos, no escenarios** (`src/xray/product/group_opportunities.py`, `scripts/14_group_opportunities.py` → `data/processed/group_opportunities/`). Tres prácticas estándar de tesorería corporativa, contadas sobre datos reales: (1) **cash pooling**: 1.147 meses-empresa con caja negativa (219 empresas, 110 grupos), el **77% cubrible con la caja de una hermana** ese mismo mes (déficit mediano 9.725 €); de los **219 meses con descubierto observado (95 empresas), el 91% tenía una hermana con caja**, con 17.823 € de coste bancario observado. (2) **netting intragrupo**: 87.638 movimientos y 42.942 M€ brutos que quedarían en 1.602 M€ netos, **96% menos**, en 153 grupos. (3) **crédito sin usar**: 42 filiales con caja negativa en el último cierre, 20 con disponible suficiente en pólizas de hermanas (mediana 939.148 €). No toca el score. Límites publicados: caja reconstruida y retrospectiva, disponible de póliza a extracción, y fiscalidad/legal fuera de los datos |
| D48 · palanca O del advisor (pagos operativos) | **Nueva y opcional** (`--levers D1,P,O`; por defecto sigue `D1,P` para no cambiar los planes publicados del compañero). La donante asume la fracción ω de los **pagos operativos** de la receptora (pagos centralizados, *payment factory*): receptora `outflow_scale(−ω)`, donante `outflow_add(+ω·o_b)`, con las mismas salvaguardas (colchón κ=2 meses, caída máxima del donante δ=5 puntos, rejilla 25/50/75/100%). **Motivación medida:** con D1 y P los bloqueos dominantes eran `recipient_no_debt_service` (3.640 pares) y `recipient_ap_component_unavailable` (3.472): las dos palancas solo tocaban deuda (25% del nivel) y retraso de pagos (15%), y **nadie tocaba el margen, que pesa el 45%**. **Resultado:** planes **19 → 48** (29 grupos nuevos, 10 con una filial por debajo de 40); 106 pasos O con importe mediano de 40.694 €; la receptora sube **9,7 puntos** de nivel (k=6) y el donante baja **0,2**; 23 de los 48 planes suben el nivel de la peor filial del grupo (media +5,7, máximo +30,7). El gasto externo del grupo no cambia: cambia quién lo soporta |
