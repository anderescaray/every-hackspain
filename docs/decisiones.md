...
### SC25 · Regeneración V2 sobre las features del ledger (19-09, noche)

`00 → 01 (D31 default) → 05 fit → 08 → 10 → 09` con `xray.ledger.classify` alimentando las features. Agosto 2026: **977 puntuadas** (281 `scored`, 696 `provisional`), 309 sin puntuar: las mismas cifras que antes del ledger, con la referencia reajustada. Prefijo 2026-02 OK en features y scores. Exportación: 1.018 empresas con ficha, 250 grupos, portfolio de 1.286, 977 con 21 escenarios; contratos válidos con `npm run validate:generated` (+ `--groups`, `--portfolio`). Suite: 343 Python + 67 frontend, lint y typecheck limpios.
