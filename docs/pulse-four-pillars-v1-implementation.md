...
## 15. Composición explícita Operating / Extended Health

`PulseFourPillars-v1.1` registra una composición nueva (`operating-extended-health-v1`) sin alterar los cuatro pilares ni la política Debt V1.0.1. La configuración y hash propios están en `pulse_four_pillars_v1_1.json` y `registry.json`; `PulseFourPillars-v1.0.1` continúa seleccionable para reproducción exacta.

- **Operating Health** = `.50 Generation + .1875 Momentum + .3125 Resilience`, sólo si los tres son válidos. Describe la dinámica operativa de caja.
- **Extended Health** = `.40 Generation + .15 Momentum + .25 Resilience + .20 Debt`, sólo si los cuatro son válidos. Debt puede estar `verified` o `bounded`, y conserva su rango de identificación. Describe además presión financiera observada e identificada.
