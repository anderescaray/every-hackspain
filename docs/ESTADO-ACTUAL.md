...
data/processed/product/
  │ 08 --production-safe (xray.group_advisor → advisor_production/; experimental sigue en advisor/)
  │ 09 export (xray.product.frontend_export → contrato JSON del frontend; grupos leen advisor_production/)
  ▼
...
4. Alertas en la ficha y en Portfolio (`alerts[]`, sin tocar componentes). **`lead_time.json` ya existe** (D41, `scripts/11_lead_time.py`): frente a estrés propio (sin embargos a terceros), V2 detecta el 32% de 142 eventos con antelación mediana de 3 meses (p25–p75 2–5), 78% de falsas alarmas y ×1,2 sobre la tasa base. Contarlo en el pitch con sus límites. Comparación de alertas en `scripts/12_early_warning.py` (D42/D43). **La mejor alerta no es el score: es la presión de deuda** (el servicio de deuda se come 5 puntos más de las entradas que su mediana de seis meses): 44% de detección con 9% de meses en alarma, ×1,8 sobre la base en la reserva de grupos y 3,5 meses de antelación, frente a 43%/14%/×1,1 de la alarma del score. **Monitor ya implementado (D44):** `scripts/13_monitor.py` → `data/processed/monitor/alerts.{parquet,json}`, 87 alertas en 85 empresas en agosto (70 presión de deuda, 17 deterioro confirmado), ordenadas por severidad y con su evidencia; conectar a `alerts[]` de la ficha y a una columna de Portfolio es trabajo de frontend. Resultados negativos registrados en D45 (hazard, CUSUM, EWMA: no mejoran). **Tesorería de grupo (D46–D48 + GA-08):** `scripts/14_group_opportunities.py` publica oportunidades medidas; `08 --levers D1,P,O` sigue siendo el laboratorio (48 planes, 6 absurdos). **Perfil seguro para la UI:** `08 --production-safe` → `advisor_production/` (19 planes / 39 pasos; 0 k=1 negativos, 0 bajadas de tramo, 0 receptoras D1/O con <10 kEUR de entradas). `09` solo mapea esos planes a `/groups/[id]/recommendations`.
5. `coverage_state` dentro del momentum de V2 (deltas solo entre meses `ok`), más allá del `provisional` actual.

**C · Si sobra tiempo**
6. Time Borrowed AP (alerta de nicho), `account_flows` en Cash Truth, Import UI. **Recomendaciones de grupo:** el perfil seguro (`08 --production-safe` → `advisor_production/`, GA-08) ya se mapea a `recommendations[]`; la red de relaciones identificadas sigue vacía en el export real.
