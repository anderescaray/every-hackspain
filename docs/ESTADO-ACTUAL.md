...
**Actionability**: `09` adapta `company_sensitivity_v1` sin recalcular el score. Separa acciones potenciales de tesorería y sensibilidades de negocio; sus estados distinguen tramo superior, arrastre operativo, ausencia de mejora y falta de datos. El impacto y el camino visibles se expresan en Health: el objetivo del tramo de **nivel V2** se traduce en Python con el momentum baseline fijo, usando la misma función del advisor. `band_basis` mantiene la semántica del tramo; no se inventa un umbral Health. La factibilidad AP corresponde al escenario seleccionado. `08_treasury_advisor.py` es requisito explícito para exportar; la ausencia de datos no se reemplaza por valores neutros. La interacción con What-if solo desplaza a su sección: las primitivas no tienen mapping exacto. Ver [`frontend-data-contract.md`](./frontend-data-contract.md).
