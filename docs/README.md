...
**Demo de esta rama `score_v2`:** el score que se publica es `PulseFourPillars-v1.2` (Operating Health principal y Extended Health cuando Debt está identificado); `financial_smoothed_v2` sigue en investigación y alimenta el advisor V2, pero no es fallback del frontend. Ver [integración Pulse](./pulse-frontend-integration.md).
...
python -X utf8 scripts/09_compute_pulse.py --as-of 2026-08-31 --data-vintage 2026-09-19
python -X utf8 scripts/09_export_frontend.py --run-dir data/processed/pulse/runs/<run_id>  # Pulse v1.2 -> snapshot web
# El exportador V2 permanece aislado en src/xray/product/legacy_frontend_export.py para investigación.
