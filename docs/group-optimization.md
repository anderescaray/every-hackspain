...
**Productización Actionability:** `src/xray/product/actionability.py` adapta los JSON de `company_sensitivity_v1` al contrato opcional de empresa en `09_export_frontend.py`. El advisor sigue siendo dueño de nivel, Health contrafactual, pendientes y rejilla. El producto escoge de esa rejilla la fila con mayor mejora positiva de Health por palanca, prioriza tesorería con el ranking de eficiencia `by_cash` y, cuando no existe, por impacto; negocio se declara sensibilidad, no recomendación. Los estados distinguen tesorería accionable, solo sensibilidad de negocio, arrastre operativo, tramo superior, ninguna mejora positiva e insuficiencia de datos. La eficiencia marginal es del **nivel V2**, mientras `health_after` proviene de `score_after_k6`; no se muestra como eficiencia de Health. La frontera del siguiente tramo es de nivel V2; el adaptador traduce esa frontera a Health visible con la **misma función del grid del advisor**, `score_with_baseline_momentum`, y conserva `band_basis=level_v2`. Momentum permanece en baseline en los contrafactuales. Solo se exponen `pts/€10k` para AP o reducción de servicio de deuda cuando hay equivalente de caja positivo e identificado; no se infiere para AR, entradas ni decisiones de negocio. Un proxy AP de stock cero no acredita liquidez requerida cero: factibilidad desconocida. El adapter no modifica ninguna fórmula V2 ni del advisor.
