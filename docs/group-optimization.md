...
**Productización Actionability:** `src/xray/product/actionability.py` adapta los JSON de `company_sensitivity_v1` al contrato opcional de empresa en `09_export_frontend.py`. El advisor sigue siendo dueño de nivel, Health contrafactual, pendientes y rejilla. El producto escoge de esa rejilla la fila con mayor mejora positiva de Health por palanca, prioriza tesorería con el ranking de eficiencia `by_cash` y, cuando no existe, por impacto; negocio se declara sensibilidad, no recomendación. La eficiencia marginal es del **nivel V2**, mientras `health_after` proviene de `score_after_k6`. El siguiente tramo también se mide en nivel V2. Momentum permanece en baseline en los contrafactuales. Solo se exponen `pts/€10k` cuando hay equivalente de caja; no se infiere para AR o entradas. El adapter no modifica ninguna fórmula V2 ni del advisor.
