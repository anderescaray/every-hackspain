...
def load_template_categories(path) -> pd.DataFrame:
    ...
    m = m.drop_duplicates()
    if m.duplicated(["tpl", "sign"]).any():
        raise ValueError("Static category cache contains conflicting template/sign evidence")
    return m.sort_values(["tpl", "sign"]).reset_index(drop=True)
