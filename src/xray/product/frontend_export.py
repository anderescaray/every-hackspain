...
def _cash_truth(company_id, group_id, cash, confidence, evidence_rows):
    ...
    own_rows = int(own.get("n_tx") or 0) if own else 0
    own_in = _round(own.get("amount_in")) if own else 0.0
    own_out = _round(own.get("amount_out")) if own else 0.0
    # Canonical debt service can take precedence over one leg of an own mirror.
    # A directional/incomplete bucket is not evidence of zero transfers or net zero.
    own_empty = own_rows == 0 and own_in == 0 and own_out == 0 and agg["circulation"]["gross"] == 0
    own_balanced = (own_rows >= 2 and own_rows % 2 == 0 and own_in is not None
                    and own_in > 0 and own_in == own_out)
    own_identified = own_empty or own_balanced
    explain = {
    ...
    if not own_identified:
        explain["circulation"] = "Circulación propia identificada parcialmente: los tramos no concilian dentro de esta categoría. No se infiere neto cero ni un número de traslados."
    components = []
    for cat in ("operating", "circulation", "support", "uncertain"):
        a = agg[cat]
        net = (0.0 if own_identified else None) if cat == "circulation" else None if cat == "uncertain" else round(a["in"] - a["out"], 2)
        conf = None if cat == "uncertain" or (cat == "circulation" and not own_identified) else _score(confidence)
    ...
    own_count = own_rows // 2 if own_identified else None
    own_amount = own_in if own_identified else None
    ...
    circulation_explanation = "tiene neto cero" if own_identified else "tiene tramos pendientes de conciliar y neto no evaluable"
    explanation = (f"En {period} la operación identificada aporta {op_net:,.0f} € netos y el apoyo intragrupo {sup_net:,.0f} €; "
                   f"la circulación entre cuentas propias {circulation_explanation} y {agg['uncertain']['gross']:,.0f} € brutos quedan sin atribuir. "
    ...
    return {
        ...
        "own_account_circulation": {"transferred_amount": own_amount, "transfer_count": own_count,
                                    ...
                                    "confidence": _score(confidence), "evidence_refs": refs if own_count else []} if own_identified else None,
