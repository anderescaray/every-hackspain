...
from xray.score.core import fit_reference_bundle, score_panel, validate_scores
from xray.score.report import example_cases, json_safe, latest_scores, score_report


def read_panel(features_dir, config):
    features_dir = Path(features_dir)
    if (features_dir / ".pipeline.lock").exists():
        raise RuntimeError("Publicación de features en curso")
    manifest_path = features_dir / "_feature_manifest.json"
    manifest_hash = sha256(manifest_path)
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    path = features_dir / config.feature_file
    digest = sha256(path)
    if manifest["outputs_sha256"].get(path.name) != digest:
        raise ValueError("Las features no coinciden con su manifiesto; regenerar o revisar la capa processed")
    frame = pd.read_parquet(path)
    return frame, {"feature_file": path.name, "feature_sha256": digest, "feature_manifest_sha256": manifest_hash}
...
def run(features_dir=PROCESSED_DIR, out_dir=None, config=None, mode="fit", reference_file=None, verbose=True):
    ...
    if mode == "predict":
        ...
        stored_config = ScoreConfig(**reference["config"])
        ...
        reference_hash = None
    features_dir = Path(features_dir)
    out_dir = Path(out_dir) if out_dir is not None else features_dir / "scores"
    ...
    with tempfile.TemporaryDirectory(prefix="scores-", dir=out_dir.parent) as directory:
        ...
        documents = {
            ...
            f"{prefix}_score_examples.json": example_cases(scores, explanations, config.unit),
        }
        ...
        manifest = {"created_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
                    "method": reference["method"], "mode": mode, "config": asdict(config), "input": source,
    ...
    if verbose:
        print(json.dumps(json_safe({"mode": mode, "out_dir": str(out_dir), "latest_month": report["latest_month"],
                                    "reference_groups": report["reference_group_count"], "holdout_groups": report["holdout_group_count"],
                                    "latest": report["latest"]}), indent=2, ensure_ascii=False, allow_nan=False))
    return report


def validate_saved_scores(scores_dir=None, features_dir=PROCESSED_DIR, panel="company", prefix_month=None):
    features_dir = Path(features_dir)
    scores_dir = Path(scores_dir) if scores_dir is not None else features_dir / "scores"
    ...
    if prefix_month is not None:
        ...
        prefix = features.loc[features.month.le(cutoff)]
        recalculated, _ = score_panel(prefix, reference)
    ...
    return {"rows": len(scores), "scored_rows": int(scores.score.notna().sum()),
