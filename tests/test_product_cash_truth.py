...
from xray.product.cash_truth import BUCKETS, classify, compute_cash_truth, dependency_summary, monthly_buckets


def tx(i, company, date, amount, category="collection", internal=False, intragroup=False, status="booked", fx=1.0, **flags):
    row = dict(transaction_id=f"T{i}", company_id=company, product_id="P1", date=pd.Timestamp(date), amount=float(amount),
...
def test_buckets_priority_and_eligibility():
    rows = [tx(1, "A", "2026-01-05", 100),                                   # operations
            ...
            tx(4, "A", "2026-01-07", -20, category="fee"),                    # financing_investment
...
def test_dependency_uses_only_past_months_and_needs_support():
    ...
    _, monthly, summary = compute_cash_truth(pd.DataFrame(rows), stop=pd.Timestamp("2026-09-01"))
