...
from xray.product.frontend_export import company_detail, run
...
def test_export_continues_without_company_sensitivity(tmp_path):
    product = tmp_path / "product"
    (product / "companies").mkdir(parents=True)
    (product / "groups").mkdir()
    (product / "portfolio.json").write_text(json.dumps({"latest_month": "2026-08-01", "companies": []}))
    import pandas as pd
    pd.DataFrame(columns=["company_id", "month", "currency"]).to_parquet(product / "cash_truth_summary.parquet")
    manifest = run(product, tmp_path / "out", advisor_dir=tmp_path / "advisor", verbose=False)
    assert manifest["companies_written"] == 0
    assert manifest["companies_with_actionability"] == 0
